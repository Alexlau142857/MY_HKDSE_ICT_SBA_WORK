...
const ll MAX_LOGIN_ATTEMPTS = 5;
const ll LOCKOUT_DURATION = 30;
...
std::map<std::string, ll> failed_counts;
...
ll bin_search (account a){
    ll l = 0, r = accounts.size() - 1;
    while (l <= r){
        ll mid = (l + r) / 2;
        if (accounts[mid].id == a.id)
            return mid;
        else if (accounts[mid].id < a.id)
            l = mid + 1;
        else
            r = mid - 1;
    } return -1;
...
std::string hind_password(const std::string &prompt)
{
    std::cout << prompt;
    std::string pass = "";
    bool ok = false;
...
bool locked(const std::string &id)
{
    if (failed_counts[id] >= MAX_LOGIN_ATTEMPTS)
    {
        ...
        if (seconds_passed < LOCKOUT_DURATION)
        {
            std::cout << ">> [LOCKED] Too many failed attempts. Try again in "
                      << (LOCKOUT_DURATION - (ll)seconds_passed) << " seconds.\n";
            return true;
        }
        else
        {
...
void login()
{
    ...
    if (input_id != "admin" && locked(input_id))
    {
        return;
    }
    std::cout << "(Press 'Tab' to show/hide password)\n";
    std::string input_pw = hind_password("Password: ");
...
void change_password()
{
    ...
    std::string old_pw = hind_password("Enter Old Password: ");
    ...
    while (true)
    {
        ...
        new_pw = hind_password("Enter New Password: ");
        if (okpassword(new_pw))
            break;
